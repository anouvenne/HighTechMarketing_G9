...
positive_terms_bic <- terms(textmodel_positive_bic_new, 10)
negative_terms_bic <- terms(textmodel_negative_bic_new, 10)
...
```
Polarity comparison <br>
Interpretation:
The p-value = 0.8287 is much greater than 0.05, so we fail to reject the null hypothesis. <br>

This means: There is no statistically significant difference in the average sentiment polarity between BIC Cristal and ReMarkable 2 Pen reviews. <br>

The 95% confidence interval includes 0 ([-0.0447, 0.0358]), which further confirms that the difference in means could very well be zero. <br>

The means (0.126 vs 0.130) are nearly identical — the difference is negligible.

Subjectivity comparison <br>
nterpretation:
The p-value = 0.2178 is also greater than 0.05, so again, we fail to reject the null hypothesis.<br>

There is no significant difference in average subjectivity between the two product reviews. <br>

Although ReMarkable reviews appear slightly more subjective on average (0.114 vs 0.107), this difference is not statistically significant. <br>
...
# Example: Add sentiment category (positive, neutral, negative)
combined <- combined %>%
...
sentiment and subjectivity analysis shows that:

BIC Cristal and ReMarkable 2 Pen reviews have similar sentiment (polarity) and subjectivity levels.

No statistically significant difference is detected between them on either metric.

Both t-tests and ANOVA are consistent in this finding.<br>
...
Since p-value = 0.9156 is much greater than 0.05, you fail to reject the null hypothesis.

This means:

There is no statistically significant difference in the distribution of positive and negative reviews between BIC Cristal and ReMarkable 2.

Conclusion
Even though BIC has slightly more positive reviews in absolute terms, the proportions are not significantly different — they could be due to random variation.

If you'd like, I can help you visualize this difference with a bar chart or compute effect sizes to complement the test.
