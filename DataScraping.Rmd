...
safe_scrape_reddit <- function(product_name) {
  tryCatch({
    ...
    search_config <- list(
      "BIC-Cristal" = list(
        terms = "BIC+Cristal+pen -meth -glass",
        subs = c("pens", "stationery", "Art", "BuyItForLife"),
        include = "bic.*cristal|ballpoint",
    ...
    content <- map_df(head(threads$url, 100), ~{    # By increasing the number you increase data sets 
      tryCatch({
...
# Keep only the first 200 unique reviews
slice_head(content %>% distinct(), n = 200)     # To obtain around 200 reviews per product
...
# CHUNK 5: DATA EXPORT
# ---------------------------
write.csv(bic_data, "bic_cristal_reddit.csv", row.names = FALSE)
write.csv(remarkable_data, "remarkable2_pen_reddit.csv", row.names = FALSE)
```
