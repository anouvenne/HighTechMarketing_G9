...
```
Now, we can proceed to the sentiment analysis. We will use the 'sentimentr' package to calculate the sentiment scores for each review in both corpora. The sentiment scores will be stored in a new variable called 'ave_sentiment' in the document variables of each corpus.
```{r sentriment analysis}
library(sentimentr)

# BIC Cristal Sentiment
sent_bic <- sentiment_by(get_sentences(as.character(corpusbic)))
docvars(corpusbic, field = "ave_sentiment") <- sent_bic$ave_sentiment

# Remarkable 2 Pen Sentiment
sent_rm <- sentiment_by(get_sentences(as.character(corpusRM)))
docvars(corpusRM, field = "ave_sentiment") <- sent_rm$ave_sentiment

```
We can check if the length of the sentiment scores matches the number of documents in each corpus. 

```{r length check}
#BIC Cristal
length(sent_bic$ave_sentiment) == ndoc(corpusbic)
#Remarkable 2 Pen
length(sent_rm$ave_sentiment) == ndoc(corpusRM)
...
```
The value indicates that sentiment for both are positive, with BIC Cristal having a mean of 0.25 and a median of 0.28, while Remarkable 2 Pen has a mean of 0.16 and a median of 0.18. Let's explore sentiments wiht other method.
...
```
We have removed the stopwords from the tokens, now we can create dfm for each corpus. A document-feature matrix (DFM) is a matrix that represents the frequency of each word (feature) in each document. We will use the `quanteda` package to create the DFM.
```{r dfm creation}
#BIC Cristal
